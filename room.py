...
class Room:
    ...
    def run(self):
        ...
        while (running):    
            ...
            for option in self.options:
                ...
                if "req" in option:
                    ...
                    for req in option["req"]:
                        ...
                        if req[0] == "!":
                            if namespace in Room.flags.keys() and req[1:] in Room.flags[namespace]:
                                break   # stop checking requirements for this option
                        # if requirement is not marked as a flag, requirement is not met
                        elif namespace not in Room.flags.keys() or (namespace in Room.flags.keys() and req not in Room.flags[namespace]):
                            break   # stop checking requirements for this option
            ...
            #
            # deal with commands in the JSON in our selected option
            #
            ...
            if "result" in option:
                ...
                for result in option["result"]:
                    ...
                    # if the command is "unset", remove a flag from the flags list if it exists
                        ...
                        if namespace in Room.flags.keys():
                            # while there are occurences of the argument flag, remove them
                            while argument in Room.flags[namespace]:
                                # remove an occurence of the argument flag
                                Room.flags[namespace].remove(argument)
                        continue    # continue to the next command
